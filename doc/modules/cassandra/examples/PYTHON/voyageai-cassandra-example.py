...
This comprehensive example demonstrates the complete VoyageAI by MongoDB integration with
Cassandra, combining multiple advanced features in one production-ready guide:

1. Standard text embeddings (voyage-4-large, voyage-4, voyage-4-lite)
...
class Config:
    ...
    EMBEDDING_MODEL = "voyage-4-lite"  # Options: voyage-4-large, voyage-4, voyage-4-lite
    RERANK_MODEL = "rerank-2.5"  # Options: rerank-3, rerank-3-lite, rerank-2.5, rerank-2.5-lite
...
# Total tokens per request (per batch) for VoyageAI by MongoDB models
VOYAGE_TOKEN_LIMITS = {
